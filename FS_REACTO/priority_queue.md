...
A _priority queue_ is a data structure that takes a priority value with each piece of data and returns the data in order of priority.
...
1. Uses a _complete_ binary tree, which means that every level of the tree is full except the bottom level, which is filled from left to right
2. Insertions satisfy the _heap-order-property_, which says that no child's value is greater than its parent's value (in a _max_ heap)
